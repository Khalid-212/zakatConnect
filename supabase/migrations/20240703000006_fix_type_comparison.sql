...
FOR ALL
TO authenticated
USING ((SELECT role FROM users WHERE id = auth.uid())::text = 'super-admin');
...
TO authenticated
USING (
  (SELECT role FROM users WHERE id = auth.uid())::text IN ('super-admin', 'admin')
);
...
TO authenticated
USING (
  (SELECT role FROM users WHERE id = auth.uid())::text = 'clerk'
);