...
export default async function AnalyticsPage() {
  ...
  if (userData?.role !== 'super-admin' && userData?.role !== 'admin') {
    return redirect('/dashboard?error=You do not have permission to access analytics');
  }
  ...
  const giversQuery = supabase.from('givers').select('*', { count: 'exact', head: true });
  ...
  const [{ count: beneficiariesCount }, { count: giversCount }] = await Promise.all([
    beneficiariesQuery,
    giversQuery,
  ]);
  ...
  const totalCollected =
    collections?.reduce((sum, item) => {
      if (item.type === 'in_kind' && item.product_type_id && item.product_types?.[0]?.price) {
        return sum + (item.amount || 0) * item.product_types[0].price;
      }
      return sum + (item.amount || 0);
    }, 0) || 0;

  const totalDistributed = distributions?.reduce((sum, item) => sum + (item.amount || 0), 0) || 0;
...
function processDailyData(collections: Collection[], distributions: Distribution[]) {
  ...
  collections.forEach((item) => {
    ...
    if (item.type === 'in_kind' && item.product_type_id && item.product_types?.[0]?.price) {
      dailyCollections[dayKey] += (item.amount || 0) * item.product_types[0].price;
    } else {
  ...
  // Get last 30 days
  const days = new Set([...Object.keys(dailyCollections), ...Object.keys(dailyDistributions)]);
...
function processProductDistribution(collections: Collection[]) {
  ...
  collections.forEach((item) => {
    if (item.type === 'in_kind' && item.product_type_id && item.product_types?.[0]) {
      const productName = item.product_types[0].name || 'Unknown';
      const productValue = (item.amount || 0) * (item.product_types[0].price || 0);
